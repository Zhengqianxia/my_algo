...
class UnityWrapperProcess:
    def __init__(self,
        ...
        self.use_rs = False  # 是否使用奖励重塑 True/False
        if self.use_rs:
            self.llm_wrapper = LLMWrapper(call_type="rs")
    ...
    def step(self, ma_d_action, ma_c_action):
        ...
        for n in self.behavior_names:
            ...
            for agent_id in decision_steps:
                ...
                if self.use_rs and info.obs[2][1] == 1.0:
                    preference_reward = self.llm_wrapper.get_value(info.obs[2], ma_c_action[n][agent_idx])  # 偏好奖励
                    ma_reward[n][agent_idx] = info.reward + preference_reward
                else:
            ...
            for agent_id in terminal_steps:
                ...
                ma_reward[n][agent_idx] = info.reward
                # preference_reward = calcu_preference_reward(info.obs[2], ma_c_action[n][agent_idx])  # 偏好奖励
                # ma_reward[n][agent_idx] = info.reward + preference_reward
            ...
            for n in self.behavior_names:
                ...
                for agent_id in decision_steps:
                    ...
                    if agent_id not in visited_ma_agent_ids[n]:
                        ma_reward[n][agent_idx] = info.reward
                        # preference_reward = calcu_preference_reward(info.obs[2], [0.0, 0.0])  # 偏好奖励
                        # ma_reward[n][agent_idx] = info.reward + preference_reward
