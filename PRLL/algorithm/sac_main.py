...
class Main:
    ...
    def __init__(self, root_dir, config_dir, args):
        ...
        self._profiler = UnifiedElapsedTimer(self._logger)  # 性能分析和计时
    ...
    def _init_sac(self, config_abs_dir: Path):
        # 遍历多智能体管理器
        for n, mgr in self.ma_manager:
            # If nn models exists, load saved model, or copy a new one 检查并加载神经网络模型
            ...
            mgr.set_rl(SAC_Base(obs_names=mgr.obs_names,
                                ...
                                **mgr.config['sac_config'],
                                ...
                                replay_config=mgr.config['replay_config']))  # 经验回放的配置

    def _run(self):
        ma_obs_list = self.env.reset(reset_config=self.reset_config)  # 环境在重置后返回的多智能体观察列表
        ...
        try:
            while iteration != self.base_config['max_iter']:
                ...
                while not self.ma_manager.is_done():
                    # ######回合开始########
                    # 使用上下文管理器 _profiler 来监控 get_ma_action 的性能，统计该过程的时间。重复 10 次
                    ...
                    # 使用 _profiler 上下文管理器来监控调用环境的 step 方法的性能。重复 10 次
                    ...
                    # 检查 ma_next_obs_list 是否为 None，如果是，则当前回合被忽略，然后继续到下一次循环
                ...
                # 如果当前不是训练迭代，则增加迭代计数 iteration，以准备进入下一个迭代
                if not training_iteration:
                    iteration += 1
